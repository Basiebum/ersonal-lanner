...
# Allow HTTP for local dev — remove in production or set via env
...
def _get_google_creds():
    """Return valid Google Credentials for the session, refreshing if needed."""
...
def _wigbo_to_gcal(ev):
    """Convert a wigbo event dict → Google Calendar event body."""
...
def _gcal_to_wigbo(gcal_ev):
    """Convert a Google Calendar event → wigbo event dict."""
...
def auth_google():
    """Kick off OAuth — redirect to Google consent screen."""
...
def auth_google_callback():
    ...
    try:
        ...
        session['google_token'] = json.loads(flow.credentials.to_json())
...
def gcal_pull():
    """Pull events from Google Calendar → return as wigbo event objects."""
...
def gcal_push():
    """Create or update one wigbo event in Google Calendar."""
    creds = _get_google_creds()
    if not creds:
        return jsonify({'error': 'not connected'}), 401
    ...
    try:
        svc      = build('calendar', 'v3', credentials=creds)
        gcal_ev  = _wigbo_to_gcal(ev)
        existing = ev.get('gcal_id')
        if existing:
            result = svc.events().update(calendarId='primary', eventId=existing, body=gcal_ev).execute()
        else:
            result = svc.events().insert(calendarId='primary', body=gcal_ev).execute()
...
def gcal_delete():
    """Delete one event from Google Calendar by gcal_id."""
    creds = _get_google_creds()
    if not creds:
        return jsonify({'error': 'not connected'}), 401
...
def gcal_sync():
    """
    Full two-way sync:
      - Push wigbo events that have no gcal_id yet
      - Pull all Google Calendar events for ±30/60 days
    Body: { events: [wigbo events to push] }
    """
    creds = _get_google_creds()
    if not creds:
        return jsonify({'error': 'not connected'}), 401
